...
// TARDEO con il QR d'ingresso monouso, non appena una riga viene
// inserita in tardeo_prenotazioni.
//
// Deploy:
//   supabase functions deploy invia-email-tardeo --no-verify-jwt
//
// Poi crea un Database Webhook (Database > Webhooks) su:
//   tabella: tardeo_prenotazioni
//   evento: Insert
...
Deno.serve(async (req: Request) => {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:0 auto;background:#0B1626;color:#F4E8CE;padding:32px 24px;border-radius:20px">
    ...
    <div style="background:#12192A;border:1px solid #223047;border-radius:16px;padding:20px;text-align:center">
      <img src="${qrImageUrl}" width="220" height="220" alt="QR ingresso" style="display:block;margin:0 auto 16px;border-radius:8px" />
      <p style="font-size:11px;color:#9FB0C9;margin:0 0 4px;text-transform:uppercase;letter-spacing:1px">Prenotazione</p>
      <p style="font-size:15px;color:#fff;font-weight:bold;margin:0 0 12px">🌙 Ingresso + drink omaggio — 10€</p>
      <p style="font-size:11px;color:#9FB0C9;margin:0">Invitato da: ${row.chi_ti_ha_invitato}</p>
    </div>

    <p style="font-size:11px;color:#9FB0C9;line-height:1.6;margin:20px 0 0">
      Questo QR è valido una sola volta ed è legato a questa prenotazione: non condividerlo,
      chi arriva per primo con il QR valido entra. Ci vediamo ${dataLabel} dalle 19:00 su
      Nello Ocean Beach, Magazzeno — DJ Set Kiki, Adolfo Citro, H8daprep e Omen, fino a notte fonda.
    </p>
  </div>`;
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM_EMAIL,
      to: row.email,
      subject: `Il tuo accesso a TARDEO (${dataLabel}) — QR in allegato`,
      html,
    }),
  });
