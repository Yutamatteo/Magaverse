...
(function () {
    ...
    // ─────────────────────────────────────────────────────────
    // COUNTDOWN — Target: 2 Giugno 2026 ore 16:00
    // ─────────────────────────────────────────────────────────
    function updateCountdown() {
        const target = new Date('2026-06-02T16:00:00+02:00');
        const diff = target - new Date();
        const pad = n => n.toString().padStart(2, '0');
        const set = (id, val) => { const el = document.getElementById(id); if (el) el.innerText = val; };
        if (diff <= 0) { ['cd', 'ch', 'cm', 'cs'].forEach(id => set(id, '00')); return; }
        set('cd', pad(Math.floor(diff / 86400000)));
        set('ch', pad(Math.floor(diff / 3600000) % 24));
        set('cm', pad(Math.floor(diff / 60000) % 60));
        set('cs', pad(Math.floor(diff / 1000) % 60));
    }
    ...
    function boot() {
        ...
        // Countdown solo se gli elementi esistono (pagina Info)
