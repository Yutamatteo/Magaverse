...
- **`staff/index.html`** — **unico ingresso staff**, per tutti i ruoli. Dopo il login instrada in automatico:
  - **bar** / **ingresso** → va dritto allo scanner, bloccato sulla propria postazione
  - **admin** / **superadmin** → due schede: **Scanner** (può scegliere bar o ingresso, utile a coprire un turno) e **Gestione** (conferma pagamenti, timbro/punti manuali, riscatto premi; la gestione soglie livelli è visibile solo al superadmin)
...
| `ingresso` | Scanner → aggiunge un timbro (max 1 al giorno) **+ valida i QR d'ingresso di Marea** (e di eventuali futuri format analoghi) |
...
> **Nota Marea**: lo stesso scanner "ingresso" ora riconosce da solo se il
> codice inquadrato è una MagaCard o un QR di prenotazione Marea — nessuna
> pagina o login separati. Il ruolo `bar` non ha accesso ai QR d'ingresso,
> solo ai punti MagaCard. Vedi `magaparty/docs/README-marea.md` per i
> dettagli lato Marea.

Un account admin/superadmin può anche usare lo scanner al posto di bar/ingresso (utile se manca qualcuno del turno): dopo il login, nella scheda Scanner, gli viene chiesto quale postazione vuole coprire.
