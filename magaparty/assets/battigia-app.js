/* BATTIGIA – Shell JS
 * Inietta sidebar/header/mobile-tabs/bottom-cta nella pagina battigia.html.
 * Countdown ricorrente (ogni mercoledì 20:00 -> 01:00), share, calendario.
 */
...
(function () {
    ...
    function logoMarkup(extraStyle) {
        return `<img src="assets/img/battigia-locandina.jpg" alt="BATTIGIA" class="logo-image"${extraStyle ? ' style="' + extraStyle + '"' : ''}
                     onerror="this.outerHTML='<div class=\\'logo-placeholder\\'><i class=\\'fa-solid fa-image\\'></i><span>battigia-locandina.jpg</span></div>'">`;
    }
        ...
        <aside class="desktop-sidebar">
            <div class="sidebar-logo-section">
                <div class="sidebar-meta">
                    <span style="font-size:9px;text-transform:uppercase;letter-spacing:0.2em;color:#6b7280">Magaparty.battigia</span>
                    <div style="display:flex;align-items:center;gap:8px;font-size:10px;letter-spacing:0.15em;font-weight:700;text-transform:uppercase;color:var(--sunset-orange)">
                        <span style="width:8px;height:8px;border-radius:9999px;background:var(--sunset-orange);animation:pulse 1.5s infinite"></span> Ogni mercoledì
                    </div>
                </div>
                ${logoMarkup()}
                <p class="brutalist" style="font-size:14px;letter-spacing:0.2em;margin-top:8px;color:var(--sunset-gold)">BATTIGIA</p>
                <p style="font-size:9px;text-transform:uppercase;letter-spacing:0.2em;opacity:0.55;margin-top:4px">Il mercoledì chill</p>
            </div>
            ...
            <div class="sidebar-cta">
                <button data-testid="sidebar-cta-lista" onclick="BATTIGIA.goToForm()" style="width:100%;color:var(--night-bg);font-weight:900;padding:12px;border-radius:var(--radius-pill);font-family:'Archivo Black',sans-serif;font-size:12px;letter-spacing:0.2em;border:none;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:8px;margin-bottom:12px;background:var(--sunset-orange);box-shadow:0 0 20px rgba(226,84,14,0.35)">
                    <i class="fa-solid fa-list-check"></i> METTITI IN LISTA
                </button>
                <div style="display:flex;gap:8px">
                    <button data-testid="sidebar-share" onclick="BATTIGIA.share()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(240,255,232,0.08);cursor:pointer">
                        <i class="fa-solid fa-share-nodes"></i> Condividi
                    </button>
                    <button data-testid="sidebar-calendar" onclick="BATTIGIA.addToCalendar()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(240,255,232,0.08);cursor:pointer">
    ...
    function buildMobileHeader() {
        ...
        <div class="mobile-header-wrapper">
            <header style="padding:40px 24px 24px;text-align:center">
                ...
                <p class="brutalist sunset-title" style="font-size:22px;letter-spacing:0.2em;margin-top:8px">BATTIGIA</p>
                <p style="font-size:10px;text-transform:uppercase;letter-spacing:0.3em;opacity:0.55;margin-top:4px">Il mercoledì chill</p>
    ...
    // ─────────────────────────────────────────────────────────
    // CARD DINAMICA — Countdown -> Festa in corso -> torna al countdown (ricorrente ogni mercoledì)
    // ─────────────────────────────────────────────────────────
    function wednesdayWindow() {
        // NOTA: fine serata stimata all'1:00 (5 ore dopo l'apertura delle 20:00).
        // Se l'orario di chiusura reale è diverso, modifica il +5 qui sotto.
        const now = new Date();
        const day = now.getDay(); // 0=dom ... 3=mercoledì
        let start = new Date(now);
        start.setHours(20, 0, 0, 0);
        start.setDate(now.getDate() - day + 3); // torna al mercoledì di questa settimana
        let end = new Date(start);
        end.setDate(end.getDate() + 1);
        end.setHours(1, 0, 0, 0); // 20:00 -> 01:00 (giorno dopo)
        if (now >= end) {
    ...
    function updateDynamicCard() {
        const now = new Date();
        const { start, end } = wednesdayWindow();
    ...
    function share() {
        if (navigator.share) {
            navigator.share({ title: 'BATTIGIA – Il Mercoledì di Magaparty', text: "Il mercoledì chill @ Nello Ocean Beach", url: window.location.href }).catch(() => {});
        } else {
    ...
    function addToCalendar() {
        ...
        const t = encodeURIComponent('BATTIGIA – Il Mercoledì di Magaparty');
        const l = encodeURIComponent('Nello Ocean Beach, Magazzeno Salerno');
        const det = encodeURIComponent("Il mercoledì chill di Magaparty. Ingresso dalle 20:00.");
