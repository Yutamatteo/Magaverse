...
(function () {
    ...
    function buildSidebar() {
        ...
        <aside class="desktop-sidebar">
            <div class="sidebar-logo-section">
                ...
                ${logoMarkup()}
                <p class="brutalist" style="font-size:14px;letter-spacing:0.2em;margin-top:8px;color:var(--sunset-gold)">MAGA REPUBLIC</p>
                <p style="font-size:9px;text-transform:uppercase;letter-spacing:0.2em;opacity:0.4;margin-top:4px">Un tramonto dedicato alla musica</p>
            </div>

            <nav class="sidebar-nav">${items}</nav>

            <div class="sidebar-cta">
                <button data-testid="sidebar-cta-lista" onclick="MAGA.openModal()" style="width:100%;color:#0d1a1a;font-weight:900;padding:12px;border-radius:12px;font-family:'Archivo Black',sans-serif;font-size:12px;letter-spacing:0.2em;border:none;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:8px;margin-bottom:12px;background:var(--sunset-orange);box-shadow:0 0 20px rgba(255,107,26,0.35)">
                    <i class="fa-solid fa-list-check"></i> METTITI IN LISTA
                </button>
                <div style="display:flex;gap:8px">
                    <button data-testid="sidebar-share" onclick="MAGA.share()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(255,248,231,0.08);cursor:pointer">
                        <i class="fa-solid fa-share-nodes"></i> Condividi
                    </button>
                    <button data-testid="sidebar-calendar" onclick="MAGA.addToCalendar()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(255,248,231,0.08);cursor:pointer">
    ...
    function buildMobileHeader() {
        ...
        <div class="mobile-header-wrapper">
            <header style="padding:40px 24px 24px;text-align:center">
                ...
                <p class="brutalist sunset-title" style="font-size:22px;letter-spacing:0.2em;margin-top:8px">MAGA REPUBLIC</p>
                <p style="font-size:10px;text-transform:uppercase;letter-spacing:0.3em;opacity:0.5;margin-top:4px">Un tramonto dedicato alla musica</p>
            </header>
        </div>`;
    }
    ...
    function buildMobileTabs() {
        ...
        return `
        <div class="mobile-sticky-tabs" style="position:sticky;top:0;z-index:40;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-bottom:1px solid rgba(255,248,231,0.05);box-shadow:0 10px 20px rgba(0,0,0,0.5);background:rgba(13,26,26,0.95)">
    ...
    function buildBottomCta() {
        return `
        <div class="bottom-cta-mobile" style="position:fixed;bottom:0;left:0;right:0;padding:16px;z-index:50;background:linear-gradient(to top, #0d1a1a, rgba(13,26,26,0.9), transparent)">
            <button data-testid="bottom-cta-lista" onclick="MAGA.openModal()" style="width:100%;color:#000;font-weight:900;padding:16px;border-radius:12px;font-family:'Archivo Black',sans-serif;font-size:18px;letter-spacing:0.2em;border:none;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:8px;background:var(--sunset-orange);box-shadow:0 0 20px rgba(255,107,26,0.35)">
                <i class="fa-solid fa-list-check"></i> METTITI IN LISTA
            </button>
        </div>`;
    }
    ...
    function buildModal() {
        return `
        <div id="listModal" data-testid="list-modal" style="position:fixed;inset:0;z-index:100;display:none;align-items:center;justify-content:center;padding:16px;background:rgba(13,26,26,0.92);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)">
            <div class="glass-card modal-enter neon-border-orange" style="width:100%;max-width:24rem;padding:24px;position:relative;background:#0d1a1a">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px">
                    <h3 class="brutalist" style="font-size:24px;color:#fff;margin:0">LISTA INGRESSO</h3>
                    <button data-testid="modal-close" onclick="MAGA.closeModal()" style="width:32px;height:32px;display:flex;justify-content:center;align-items:center;border-radius:8px;background:rgba(255,255,255,0.1);color:#fff;border:none;cursor:pointer">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div style="border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:20px;margin-bottom:20px;text-align:center;background:rgba(255,255,255,0.03)">
                    ...
                    <a data-testid="modal-google-form" href="https://docs.google.com/forms/d/1fRTPOhj_Pdncv2yql9a86qWowh2NdOVZSb-bNPIaTcA/viewform" target="_blank"
                       style="display:block;width:100%;padding:12px;background:#fff;color:#000;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:0.2em;border-radius:8px;text-decoration:none">
                ...
                <div style="text-align:center;position:relative;margin:24px 0">
                    ...
                    <span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);padding:0 12px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;color:#6b7280;background:#0d1a1a">Oppure contatta i PR</span>
                </div>
                <button data-testid="modal-wa-info" onclick="MAGA.wa('Informazioni Generali')" style="width:100%;display:flex;align-items:center;justify-content:center;gap:8px;padding:12px;background:transparent;border:1px solid var(--sunset-orange);color:var(--sunset-orange);border-radius:8px;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:0.2em;margin-top:8px;cursor:pointer;box-shadow:0 0 12px rgba(255,107,26,0.2)">
    ...
    // ─────────────────────────────────────────────────────────
    // CARD DINAMICA — Countdown / DJ Live / Dopo evento
    // Evento: 2 Giugno 2026, 16:00 → 00:30 (+1)
    // ─────────────────────────────────────────────────────────
    const LINEUP = [
        { name: 'DJ KIKI',      start: [18,  0], end: [19,  0] },
        { name: 'H8DAPREP',     start: [19,  0], end: [20, 30] },
        { name: 'ADOLFO CITRO', start: [20, 30], end: [23, 30] },
        { name: 'DJ KIKI',      start: [23, 30], end: [24, 30] },
    ];

    function toMinutes(h, m) { return h * 60 + m; }

    function getCurrentDJ(now) {
        const eventDay = new Date('2026-06-02T00:00:00+02:00');
        const minFromMidnight = (now - eventDay) / 60000;
        for (let i = 0; i < LINEUP.length; i++) {
            const s = toMinutes(...LINEUP[i].start);
            const e = toMinutes(...LINEUP[i].end);
            if (minFromMidnight >= s && minFromMidnight < e) {
                return { current: LINEUP[i], next: LINEUP[i + 1] || null };
            }
        }
        return null;
    }
    ...
    function updateDynamicCard() {
        const now = new Date();
        const eventStart = new Date('2026-06-02T16:00:00+02:00');
        const eventEnd   = new Date('2026-06-03T00:30:00+02:00');
        ...
        if (now < eventStart) {
            ...
            const dj = getCurrentDJ(now);
            if (dj) {
                setText('live-dj-name', dj.current.name);
                const [sh, sm] = dj.current.start;
                const [eh, em] = dj.current.end;
                const fmt = (h, m) => pad(h % 24) + ':' + pad(m);
                setText('live-dj-time', fmt(sh, sm) + ' – ' + fmt(eh, em));
                setText('live-next-dj', dj.next ? dj.next.name : '—');
            }
        } else {
    ...
    // ─────────────────────────────────────────────────────────
    function wa(lista) {
        const msg = encodeURIComponent(`Ciao! Vorrei registrarmi per il MagaRepublic (2 Giugno 2026).\nRichiesta per: ${lista}`);
        window.open(`https://wa.me/393454237269?text=${msg}`, '_blank');
    }
    function share() {
        if (navigator.share) {
            navigator.share({ title: 'MAGA REPUBLIC – 2 Giugno | Magaparty', text: 'Un tramonto dedicato alla musica', url: window.location.href }).catch(() => {});
        } else {
            navigator.clipboard.writeText(window.location.href).then(() => showToast('Link copiato!'));
        }
    }
    function addToCalendar() {
        const t = encodeURIComponent('MAGA REPUBLIC – Magaparty');
        const l = encodeURIComponent('Nello Ocean Beach, Magazzeno Salerno');
        const det = encodeURIComponent('Un tramonto dedicato alla musica. 3 DJ. Dalle 16:00 a mezzanotte.');
        window.open(`https://calendar.google.com/calendar/r/eventedit?text=${t}&dates=20260602T160000/20260603T000000&location=${l}&details=${det}`, '_blank');
    }
    ...
    const UPLOAD_PRESET = 'ml_MagaRepublic';
    let filesToUpload = [];
    ...
    function handleDragLeave() {
        const z = document.getElementById('dropZone');
        if (z) z.style.borderColor = 'rgba(255,107,26,0.3)';
    }
    ...
    async function uploadAll() {
        ...
        if (results.length) {
            ...
            if (resList) resList.innerHTML = results.map(r => `
                <div class="glass-card" style="padding:12px 16px;display:flex;align-items:center;gap:12px;border:1px solid rgba(42,191,176,0.2);background:rgba(42,191,176,0.05)">
