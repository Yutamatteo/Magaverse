/* TARDEO – Shell JS
 * Inietta sidebar/header/mobile-tabs/bottom-cta nella pagina tardeo.html.
 * Evento unico (non ricorrente): countdown fino al 24 settembre 2026, ore 19:00
...
(function () {
    ...
    // ─────────────────────────────────────────────────────────
    const NAV = [
        { id: 'info',      href: '#info',      icon: 'fa-circle-info',   label: 'INFO',       mobileIcon: 'fa-moon' },
    ...
    function logoMarkup(extraStyle) {
        return `<img src="assets/img/tardeo-locandina.jpg" alt="TARDEO" class="logo-image"${extraStyle ? ' style="' + extraStyle + '"' : ''}
                     onerror="this.outerHTML='<div class=\\'logo-placeholder\\'><i class=\\'fa-solid fa-image\\'></i><span>tardeo-locandina.jpg</span></div>'">`;
    }
        ...
        <aside class="desktop-sidebar">
            <div class="sidebar-logo-section">
                <div class="sidebar-meta">
                    <span style="font-size:9px;text-transform:uppercase;letter-spacing:0.2em;color:#6b7280">Magaparty.tardeo</span>
                    <div style="display:flex;align-items:center;gap:8px;font-size:10px;letter-spacing:0.15em;font-weight:700;text-transform:uppercase;color:var(--sunset-orange)">
                        <span style="width:8px;height:8px;border-radius:9999px;background:var(--sunset-orange);animation:pulse 1.5s infinite"></span> 24 Settembre
                    </div>
                </div>
                ${logoMarkup()}
            ...
            <div class="sidebar-cta">
                <button data-testid="sidebar-cta-lista" onclick="TARDEO.goToForm()" style="width:100%;color:var(--night-bg);font-weight:900;padding:12px;border-radius:var(--radius-pill);font-family:'Archivo Black',sans-serif;font-size:12px;letter-spacing:0.2em;border:none;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:8px;margin-bottom:12px;background:var(--sunset-orange);box-shadow:0 0 20px rgba(226,84,14,0.35)">
                    <i class="fa-solid fa-list-check"></i> METTITI IN LISTA
                </button>
                <div style="display:flex;gap:8px">
                    <button data-testid="sidebar-share" onclick="TARDEO.share()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(200,210,230,0.08);cursor:pointer">
                        <i class="fa-solid fa-share-nodes"></i> Condividi
                    </button>
                    <button data-testid="sidebar-calendar" onclick="TARDEO.addToCalendar()" class="glass-card" style="flex:1;padding:8px;font-size:9px;text-transform:uppercase;letter-spacing:0.2em;font-weight:700;display:flex;justify-content:center;align-items:center;gap:4px;color:#fff;border:1px solid rgba(200,210,230,0.08);cursor:pointer">
    ...
    function buildMobileTabs() {
        ...
        return `
        <div class="mobile-sticky-tabs" style="position:sticky;top:0;z-index:40;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-bottom:1px solid rgba(200,210,230,0.05);box-shadow:0 10px 20px rgba(0,0,0,0.5);background:rgba(10,14,22,0.95)">
    ...
    function buildBottomCta() {
        return `
        <div class="bottom-cta-mobile" style="position:fixed;bottom:0;left:0;right:0;padding:16px;z-index:50;background:linear-gradient(to top, var(--night-bg), rgba(10,14,22,0.9), transparent)">
            <button data-testid="bottom-cta-lista" onclick="TARDEO.goToForm()" style="width:100%;color:var(--night-bg);font-weight:900;padding:16px;border-radius:var(--radius-pill);font-family:'Archivo Black',sans-serif;font-size:18px;letter-spacing:0.2em;border:none;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:8px;background:var(--sunset-orange);box-shadow:0 0 20px rgba(226,84,14,0.35)">
                <i class="fa-solid fa-list-check"></i> METTITI IN LISTA
            </button>
        </div>`;
    }
    ...
    // ─────────────────────────────────────────────────────────
    const EVENT_START = new Date('2026-09-24T19:00:00+02:00');
    const EVENT_END = new Date('2026-09-25T02:00:00+02:00');
