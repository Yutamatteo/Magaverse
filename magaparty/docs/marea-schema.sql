...
create table if not exists marea_prenotazioni (
  ...
  instagram text,
  composizione text not null,                    -- es. "2 Uomini, 3 Donne"
  email text not null,
...
begin
  ...
  if v_stato = 'usato' then
    return query
      select 'gia_usato'::text, r.nome_capogruppo, r.composizione, r.opzione, r.data_evento
  ...
  return query
    select 'ok'::text, r.nome_capogruppo, r.composizione, r.opzione, r.data_evento
    from marea_prenotazioni r where r.id = v_id;
end;
