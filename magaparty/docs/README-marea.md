...
- **`/magaparty/marea.html`** → `magaverse.it/magaparty/marea` — landing
  ...
  backup.
- **`/magaparty/marea/staff/index.html`** → `magaverse.it/magaparty/marea/staff`
  — pagina di scansione per lo staff in cassa/ingresso. Login con
  email/password, poi fotocamera sempre attiva: ogni scan valida il QR e,
  se è la prima volta, lo marca come "usato".
...
3. Allo scan, lo staff chiama la funzione `valida_ingresso_marea(qr_token)`:
   - se il token non esiste → **QR non valido**
...
Le pagine sono già puntate sullo stesso progetto di MagaCard
(`SUPABASE_URL` / `SUPABASE_ANON_KEY` in cima a `marea.html` e
`marea/staff/index.html`). Se vuoi un progetto separato, sostituisci
quelle due costanti in entrambi i file.
...
**3. Crea l'account staff per l'ingresso Marea**
Da *Authentication > Users* crea un account (email + password), poi nel
SQL Editor assegna il ruolo (sezione 5 dello schema):
```sql
update auth.users set raw_user_meta_data = raw_user_meta_data || '{"ruolo":"marea"}'::jsonb
  where email = 'ingresso.marea@magaverse.it';
```
Un account `admin` o `superadmin` già esistente (da MagaCard) funziona
automaticamente anche qui, senza bisogno di aggiungere nulla.
...
Sostituisci/aggiungi la cartella `magaparty/` e la cartella `supabase/`
...
3. La domenica, il cliente mostra il QR (schermo o email) allo staff in
   cassa
4. Lo staff apre `/magaparty/marea/staff`, fa login e scansiona: se il QR
   è valido e mai usato, l'accesso è consentito e il QR si marca "usato";
   se viene ripresentato (screenshot condiviso, doppio accesso), lo
   scanner lo segnala come già utilizzato
